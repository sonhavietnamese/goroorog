...
import type { VFXEmitterSettings } from './vfx-emitter'
...
export const VFXBuilderEmitter: React.FC<VFXBuilderEmitterProps> = ({ settings, onChange, onRestart }) => {
  ...
  if (vfxSettings.nbColors > 1) builtSettings.colorStart.push(vfxSettings.colorStart2)
  if (vfxSettings.nbColors > 1) builtSettings.colorEnd.push(vfxSettings.colorEnd2)
  if (vfxSettings.nbColors > 2) builtSettings.colorStart.push(vfxSettings.colorStart3)
  if (vfxSettings.nbColors > 2) builtSettings.colorEnd.push(vfxSettings.colorEnd3)
    ...
    if (settings) {
      ...
      for (let i = 0; i < 2; i++) {
        if (settings.colorStart && settings.colorStart.length > i) {
          builderSettings[i === 0 ? 'colorStart' : `colorStart${i + 1}`] = settings.colorStart[i]
          builderSettings.nbColors = i + 1
        }
        if (settings.colorEnd && settings.colorEnd.length > i) {
          builderSettings[i === 0 ? 'colorEnd' : `colorEnd${i + 1}`] = settings.colorEnd[i]
        }
      }
